...
\[
    \mathbb{P}(A) = \sum_{i=1}^{k} \mathbb{P}(B_i \cap A) = \sum_{i=1}^{k} \mathbb{P}(B_i)\mathbb{P}(A \mid B_i)
\]

\subsection{Bayes Theorem}
\[
    \mathbb{P}(B_i \mid A) = \frac{\mathbb{P}(B_i)\mathbb{P}(A \mid B_i)}{\mathbb{P}(A)}
\]
In Bayesian Inference,
\[
    \mathbb{P}(H \mid E) = \frac{\mathbb{P}(H)\mathbb{P}(E \mid H)}{\mathbb{P}(E)}
\]
where $\mathbb{P}(H \mid E)$ is \textbf{Posterior}, $\mathbb{P}(H)$ is \textbf{Prior}, $\mathbb{P}(E \mid H)$ is \textbf{Likelihood}, $\mathbb{P}(E)$ is \textbf{Marginal Likelihood}.
...
\begin{itemize}
    ...
    \item Bias-Variance Decomposition: $ \mathbb{E}[(X-a)^2] = (\mathbb{E}[X]-a)^2 + \operatorname{Var}(X) $
...
\[
    \mathbb{E}_{X,Y}[X] = \mathbb{E}_Y[\mathbb{E}_{X \mid Y}[X \mid Y]]
\]

\subsubsection{Jensen's Inequality}
For any convex function $f: X \to \mathbb{R}$:
\[
...
\subsection{Variance}
\[
    \sigma^2 = \operatorname{Var}(X) = \mathbb{E}[(X - \mathbb{E}[X])^2] = \mathbb{E}[X^2] - \mathbb{E}[X]^2
\]
\textbf{Properties:}
\begin{itemize}
    \item $ \operatorname{Var}(c) = 0 $
    \item $ \operatorname{Var}(cX) = c^2 \operatorname{Var}(X) $
    \item $ \operatorname{Var}(X+c) = \operatorname{Var}(X) $
...
\[
    \operatorname{Var}(X) = \mathbb{E}_Y[\operatorname{Var}(X \mid Y)] + \operatorname{Var}_Y(\mathbb{E}[X \mid Y])
\]
...
\[
    \operatorname{cov}(\mathbf{X}) = \Sigma = E[(\mathbf{X} - E[\mathbf{X}])(\mathbf{X} - E[\mathbf{X}])^{\mathrm{T}}]
\]
\textbf{Properties:}
\begin{itemize}
    \item $\operatorname{cov}(A\mathbf{X} + b) = A \, \operatorname{cov}(\mathbf{X}) \, A^{\mathrm{T}}$
\end{itemize}
...
\]
\[
    \mathbb{P}(|X-\mathbb{E}[X]| < \varepsilon) \ge 1 - \frac{\operatorname{Var}(X)}{\varepsilon^2}
\]
...
\subsubsection{Parameters and Moments}
\textbf{Mean:}
\[
    \mu = \mu_1'
\]
...
\[
    \varphi_X(t) = \mathbb{E}[e^{itX}] = \sum_{n=0}^{\infty} \frac{\mu_n' (it)^n}{n!}
\]
Relationship with moments:
\[
    \mu_n' = \frac{1}{i^n} \left.\frac{d^n}{dt^n} \varphi_X(t)\right|_{t=0}
\]
...
\[ X \sim \text{Bernoulli}(p) = B(1,p)\]
\[ p(x) = p^x (1-p)^{1-x}, \quad x \in \{0,1\} \]
\[ \mu = p \]
\[ \sigma^2 = p(1-p) \]

\subsubsection{Categorical Distribution}
Probability of observing one of $k$ mutually exclusive categories in a single trial.
\[ \mathbf{X} \sim \text{Categorical}(\mathbf{p}) = M(1, \mathbf{p})\]
Let $\mathbf{x} \in \{\mathbf{e}_i\}_{i=1}^{k}$, $\mathbf{p} \in [0,1]^k$. $\displaystyle \sum_{i=1}^{k} p_i = 1$.
\[ p(\mathbf{x}) = \prod_{i=1}^{k} p_i^{x_i} \]
\[ \boldsymbol{\mu} = \mathbf{p} \]
\[ \Sigma_{i,j} = \begin{cases} p_i(1-p_i) & i=j \\ -p_i p_j & i \neq j \end{cases} \]
...
The number of counts for $k$ mutually exclusive categories in $n$ independent trials.
\[ \mathbf{X} \sim M(n, \mathbf{p}) \]
Let $\mathbf{x} \in \{0,1,\dots,n\}^k$, $\mathbf{p} \in [0,1]^k$. $\displaystyle \sum_{i=1}^{k} x_i = n$, $\displaystyle \sum_{i=1}^{k} p_i = 1$.
\[ p(\mathbf{x}) = \frac{n!}{\prod_{i=1}^{k} x_i!} \prod_{i=1}^{k} p_i^{x_i} \]
\[ \mathbf{\mu} = n \mathbf{p} \]
\[ \Sigma_{i,j} = \begin{cases} n p_i (1 - p_i) & i=j \\ -p_i p_j & i \neq j \end{cases} \]

\subsubsection{Geometric Distribution}
The number of trials needed to get the first success in a sequence of independent Bernoulli trials.
\[ X \sim G(p) \]
\[ p(x) = (1-p)^{x-1}p, \quad x \in \mathbb{Z}^+ \]
\[ F(x) = 1 - (1-p)^x \]
\[ \mu = \frac{1}{p} \]
...
\[ p(x) = \frac{\lambda^x e^{-\lambda}}{x!}, \quad \lambda>0, \, x \in \mathbb{Z}_{\geq 0} \]
\[ \mu = \sigma^2 = \lambda \]
...
\subsubsection{Standard Univariate Normal Distribution}
...
\[ \mathbf{X} \sim \mathcal{N}(\boldsymbol{\mu}, \sigma^2 I) \]
\[
...
Let $p(x) = \mathbb{P}(X=x)$, $x \in \mathcal{X}$, $X \sim p(x)$.

\subsection{Self-Information}
The \textbf{Self-Information} (or surprisal) of an outcome $x$ is defined as:
\[
    I(x) = -\log_2 p(x)
\]

\subsection{Entropy}
The \textbf{Entropy} of $X$ measures the average uncertainty (or surprisal) of the random variable's outcome.
\[
    H(X) = \mathbb{E}[I(X)] = \mathbb{E}[-\log_2 p(X)] = -\sum_{x \in \mathcal{X}} p(x) \log_2 p(x)
\]

\subsection{Cross-Entropy}
Let $P \sim p(x)$, $Q \sim q(x)$.
\newline
\textbf{Cross-Entropy} measures the average surprisal of events drawn from distribution $P$, with a hypothesis distribution $Q$.
\[
    H(P, Q) = \mathbb{E}_p[I_q(X)] = -\sum_{x \in \mathcal{X}} p(x) \log_2 q(x)
\]

\subsection{Kullback-Leibler (KL) Divergence}
The \textbf{KL Divergence} (or relative entropy) of $P$ from $Q$ measures the average excess cause by the divergence of $P$ from $Q$.
\[
    D_{KL}(P||Q) = H(P, Q) - H(P)
\]
\[
