...
Let $\mathbf{x} = (x_1, \dots, x_k)^\top$, $\mathbf{p} = (p_1, \dots, p_k)^\top$. $\displaystyle \sum_{i=1}^{k} x_i = n$, $\displaystyle \sum_{i=1}^{k} p_i = 1$.
\[ p(\mathbf{x}) = \frac{n!}{\prod_{i=1}^{k} x_i!} \prod_{i=1}^{k} p_i^{x_i} \]
\[ \mathbf{\mu} = n \mathbf{p} \]
\[ \sigma_i^2 = n p_i (1 - p_i) \]
\[ \Sigma_{i,j} = -n p_i p_j, \quad i \neq j \]
